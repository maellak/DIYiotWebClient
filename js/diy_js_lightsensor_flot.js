...
diy_tools.prototype.wss_connect = function()  {
	...
	var conn = new ab.Session(this.wss_url+'?access_token='+this.access_token,
		function() {
			...
			var plot = $.plot($("#plotcontainer"), [{data: [], label: "Photo Resistance"}]);
			...
			conn.subscribe(subject.device, function(topic, data) {
				...
				plot.setData([{data: subject.datatime, label: "Photo Resistance"}]);
				plot.setupGrid();
				plot.draw();
				
				//add to list
				$( "#dataDev" ).append( subject.i + ". " + subject.datatime[subject.i-1][0] + " s, " + subject.datatime[subject.i-1][1] + " kOhm<br>" );
				//---plot code end---
			});
		},
		function() {
